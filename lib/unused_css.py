...
class UnusedCss:
    ...
    def get_unused_global_css(self, style_files: tuple, html: str):
        ...
        """
        modules_class_list = {file:
                              get_classes_in_css(file) for file in style_files}
    ...
    def get_unused_css(self) -> tuple:
        ...
        not_component_style_files = tuple(
            filter(self.is_not_component, source_code_style_files))

        all_html = merge_files_content(source_code_html)
        self.get_unused_global_css(not_component_style_files, all_html)
        self.files, self.count = self.framework.get_unused_component_css(
            source_code_style_files, self.files, self.count)
