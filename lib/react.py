...
class React:
    ...
    def get_unused_component_css(self, style_files, files, count) -> tuple:
        ...
        component_class_list = {file: get_classes_in_css(
            file) for file in components_css}

        for extension in self.component_extension:
            for style_file in components_css:
                file_without_extension, _ = os.path.splitext(style_file)
                # Due to ".module." in file name
                html_file_path = file_without_extension[: -7] + "." + extension
                try:
